...
/** These exercises will teach you how to work with the `List` data structure in
  * Scala in a declarative manner. At the end of these exercises, you should
  * have a good intuition on when to use `map`, `filter` or `fold`.
  */
...
/** A `List` in Scala is a linked list.
  *
  * sealed trait List[+A] case class ::[A](head: A, tail: List[A]) extends
  * List[A] case object Nil extends List[Nothing]
  *
  * You can create a List using the `::` constructor as such:
  *
  * scala> `::`(1, `::`(2, `::`(3, Nil))) // backticks to use :: in prefix
  * position > List(1, 2, 3)
  *
  * scala> (1 :: (2 :: (3 :: Nil))) // no need for backticks to use :: in infix
  * position > List(1, 2, 3)
  *
  ...
  *
  * The `apply` method in any object is the default method, so you can leave it
  * out when using it:
  *
  * scala> List(1, 2, 3)
  */

/** scala> prependToList(1, List(2, 3, 4)) > List(1,2,3,4)
  *
  * Hint: Refer the construction of list
  */
def prependToList[A](x: A, xs: List[A]): List[A] = ???

/** scala> appendToList(1, List(2, 3, 4)) > List(2,3,4,1)
  *
  * Hint: Use the :+ operator
  */
def appendToList[A](x: A, xs: List[A]): List[A] = ???

/** `List` has an `.isEmpty` method that you can call to know whether an
  * instance is empty or not.
  *
  * For this exercise, let's build a version of `isEmpty` called `isEmptyList`
  * without using `.isEmpty` (that would be cheating!).
  *
  * scala> isEmptyList(Nil) > true
  *
  * scala> isEmptyList(List(1, 2, 3)) > false
  *
  * Hint: Use pattern matching. You can pattern match on `List` using its two
  * constructors `::` and `Nil` as such:
  *
  * ```
  * list match {
...
/** scala> showListSize(List(1, 2, 3)) > "This is a list of size 3"
  *
  * scala> showListSize(List("ABC")) > "This is a list of size 1"
  *
  * scala> showListSize(Nil) > "This is an empty list"
  *
...
/** Mapping a function over a List
  ...
  *
  * scala> addNumToEach(10, List(1, 2, 3)) > List(11, 12, 13)
  *
  * Hint: Use .map
  */
...
/** Filter a List
  *
  * This is typically what you want if the size of the resulting List is <= that
  * of the initial.
  *
  * scala> filterEven(List(1, 2, 3, 4)) > List(2, 4)
  *
...
/** scala> product(List(2, 5, 3)) > 30
  *
  * https://en.wikipedia.org/wiki/Empty_product scala> product(Nil) > 1
  *
  * Hint: Use .foldLeft
  */
def product(nums: List[Int]): Int = ???

/** scala> min(List(4, 6, 1)) > 1
  *
  * scala> min(Nil) > Int.MinValue
  *
...
/** Return the person in the List that is the youngest. If there are more than
  * one person with the youngest age, return the first one.
  *
  * scala> youngestPerson(peopleList) > Person("Karen Page", 27)
  *
  * scala> youngestPerson(Nil) > Person("Nobody", 0)
  *
  * Hint: Use pattern matching and .foldLeft
  */
...
/** Return a list of pairs of a Person and their position in the `peopleList`.
  ...
  * ```
  *
  * Otherwise, you'll need to use `._1` and `._2` methods to access the fields
  * in the pair, e.g.
  *
  * ```
...
/** Log every nth person from the `peopleList` given an index `n`.
  *
  * scala> showEveryNthPerson(2, peopleList) > List("Karen Page is 27 years
  * old", "Claire Temple is 32 years old", "Elektra Natchios is 27 years old")
  *
  * Validation rules:
  *
  * If `n` is zero or less then return the full List If `n` is greater than the
  * length of the list then return an empty List
  *
...
/** Rewrite this function that uses a mutable variable and for-loop in an
  * immutable fashion
  */
...
/** Rewrite this function that uses mutable variables and for-loop in an
  * immutable fashion
  *
...
/** Pack consecutive duplicates of list elements into sublists. If a list
  ...
  *
  * Given: val l1 = List("a", "a", "a", "a", "b", "c", "c", "a", "a", "d", "e",
  * "e", "e", "e") sublists(l1) == List(List("a", "a", "a", "a"), List("b"),
